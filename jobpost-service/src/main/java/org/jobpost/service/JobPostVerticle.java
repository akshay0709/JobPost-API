...
import org.jobpost.service.model.JobPost;
...
public class JobPostVerticle extends AbstractVerticle {
		...
		router.post("/jobs").produces("application/json").handler(routingContext -> {
			...
			producer.write(rec, resp -> {
				...
				if (resp.succeeded()) {
					...
					logger.info("Saved record is: destination={}, partition={}, offset={}", recMetadata.getTopic(),
							recMetadata.getPartition(), recMetadata.getOffset());
					jobPost.setId(recMetadata.getOffset());
				} else {
					Throwable t = resp.cause();
					logger.error("Could not send topic: {}", t.getMessage());
				}
	...
	private void saveJobs(RoutingContext routingContext) {
		logger.info("Calling Savejobs");
		Map<Integer, JobPost> jobsMap = new LinkedHashMap<>();
		JsonObject jobject = routingContext.getBodyAsJson();
		jobject.getString("jobName");
		logger.info("this is a job name " + jobject.getString("jobName"));
	}
}
